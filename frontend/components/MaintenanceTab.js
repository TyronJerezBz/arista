// Maintenance Tab Component - Switch Restart & Config Management
...
export default {
    ...
    async uploadConfig() {
      ...
      try {
        ...
        if (response.data.success) {
          this.showMessage('Configuration uploaded successfully', 'success');
  ...
  template: `
    <div class="maintenance-tab">
      <!-- Messages -->
      <div v-for="msg in messages" :key="msg.message" :class="['alert', 'alert-' + msg.type, 'alert-dismissible', 'small']">
        {{ msg.message }}
        <button type="button" class="btn-close btn-sm" @click="messages = messages.filter(m => m !== msg)"></button>
      </div>
      ...
      <div class="d-flex gap-2 mb-3 flex-wrap">
        ...
        <button 
          :class="['btn btn-sm', activeTab === 'firmware' ? 'btn-primary' : 'btn-outline-secondary']"
          @click="activeTab = 'firmware'"
        >
          <i class="fas fa-hdd me-2"></i>Firmware
        </button>
        <button 
          :class="['btn btn-sm', activeTab === 'logs' ? 'btn-primary' : 'btn-outline-secondary']"
        ...
        <button 
          ...
          :disabled="true"
          title="Time settings temporarily disabled"
        >
      ...
      <!-- Time Tab (disabled button, hidden content) -->
      <div v-show="false">
        <time-settings
          :switch-id="switchId"
          :csrf-token="csrfToken"
